...
def validate_imports(start_dir):
    ...
    for root, dirs, files in os.walk(project_root):
        # Skip hidden directories like .git, .gemini, etc.
        dirs[:] = [d for d in dirs if not d.startswith('.')]
        
        for file in files:
            if file.endswith(".py") and file != "validate_imports.py":
                file_path = os.path.join(root, file)
                module_name = os.path.relpath(file_path, project_root).replace(os.path.sep, ".")[:-3]
                
                # specific excludes if needed (e.g. scripts that run main)
                if module_name in ["main", "adk_app", "demo_adk", "dashboard", "setup"]: 
                    continue 
