...
- **Regime-direction multipliers** (`_DEFAULT_REGIME_DIR_MULTIPLIERS` + `_load_regime_dir_multipliers()` in `project_lead.py`): threshold multipliers per (regime, direction) cell, bootstrapped from 336 shadow decisions (2026-06-22). Auto-reloaded from `shadow_report.json` every 30 min — cells with n≥15 samples override the defaults. Multiplier applied BEFORE the SHORT ×0.60 discount. Bad cells (×2–3) raise the bar without hard-blocking; good cells (×0.85–0.90) lower it. Logged as `[ShadowMult]`. Default values: VOLATILE+LONG ×3.0, RANGING+SHORT ×2.0, TRENDING_BEAR+LONG ×1.5, TRENDING_BULL+SHORT ×1.5, RANGING+LONG ×0.90, VOLATILE+SHORT ×0.85.
...
3. **Run `/review` audit** when: non-obvious bug fixes, new files/state files, changes to exchange_client/risk_manager/execution_agent, multiple deployments. Present report + suggested AGENTS.md/memory updates. Do NOT auto-save.
