"""
Same 3-strategy backtest on XYZ stocks/commodities.
Grouped: Commodities, Indices, Tech Stocks.
"""
import ccxt, pandas as pd, numpy as np, warnings
warnings.filterwarnings('ignore')

# Top liquid XYZ tickers, grouped
GROUPS = {
    'Commodities': ['XYZ-CL/USDC','XYZ-BRENTOIL/USDC','XYZ-GOLD/USDC','XYZ-SILVER/USDC'],
    'Indices':     ['XYZ-SP500/USDC','XYZ-XYZ100/USDC'],
    'Tech Stocks': ['XYZ-MU/USDC','XYZ-SNDK/USDC','XYZ-NVDA/USDC','XYZ-INTC/USDC',
                    'XYZ-TSLA/USDC','XYZ-AMD/USDC'],
}
ALL_TICKERS = [t for group in GROUPS.values() for t in group]
...
# --- fetch all ---
print('Fetching 62 days of XYZ data...')
raw_data = {}
for t in ALL_TICKERS:
    df = fetch(t, 62)
    if not df.empty and len(df) > 100:
        raw_data[t] = add_indicators(df)
        print(f'  {t:<30} {len(raw_data[t])} candles')
    else:
        print(f'  {t:<30} SKIPPED (insufficient data)')

# --- run for each time window ---
for window_days, label in [(60, '60 DAYS'), (30, '30 DAYS'), (7, '7 DAYS')]:
    cutoff = window_days * 24 + 50
    print(f'\n{"="*72}')
    print(f'LAST {label} -- XYZ STOCKS & COMMODITIES')
    print('='*72)

    # Per group
    for group_name, group_tickers in GROUPS.items():
        group_data = {t: raw_data[t] for t in group_tickers if t in raw_data}
        if not group_data:
            continue

        print(f'\n  [{group_name}]')
        print(f'  {"Strategy":<25} {"AvgPnL%":>8} {"AvgWR%":>7} {"AvgPF":>7} {"Trades":>7}')
        print(f'  {"-"*58}')
        ...
        for strat_name, sig_func in STRATEGIES:
            ...
            summary.append((strat_name, sum(pnls)/len(pnls), sum(wrs)/len(wrs),
                            sum(pfs)/len(pfs), sum(ns)))

        baseline_pnl = next((x[1] for x in summary if x[0] == 'Agent_Baseline'), 0)
        for name, pnl, wr, pf, nt in sorted(summary, key=lambda x: -x[1]):
            delta = pnl - baseline_pnl
            flag  = '  <<<' if name != 'Agent_Baseline' and delta > 0 else ''
            print(f'  {name:<25} {pnl:>8.1f}% {wr*100:>6.0f}% {pf:>7.2f} {nt:>7}  {delta:+.1f}pp{flag}')

    # Grand total (all XYZ tickers)
    print(f'\n  [ALL XYZ combined]')
    print(f'  {"Strategy":<25} {"AvgPnL%":>8} {"AvgWR%":>7} {"AvgPF":>7} {"Trades":>7}')
    print(f'  {"-"*58}')
    all_summary = []
    for strat_name, sig_func in STRATEGIES:
        ...
        for ticker, df_full in raw_data.items():
            ...
            pnls.append(r['pnl']); wrs.append(r['wr']); pfs.append(r['pf']); ns.append(r['n'])
        if not pnls: continue
        all_summary.append((strat_name, sum(pnls)/len(pnls), sum(wrs)/len(wrs),
                        sum(pfs)/len(pfs), sum(ns)))
    baseline_pnl = next((x[1] for x in all_summary if x[0] == 'Agent_Baseline'), 0)
    for name, pnl, wr, pf, nt in sorted(all_summary, key=lambda x: -x[1]):
        delta = pnl - baseline_pnl
        flag  = '  <<<' if name != 'Agent_Baseline' and delta > 0 else ''
        print(f'  {name:<25} {pnl:>8.1f}% {wr*100:>6.0f}% {pf:>7.2f} {nt:>7}  {delta:+.1f}pp{flag}')

    # Best strategy per-ticker breakdown
    best_name = sorted(all_summary, key=lambda x: -x[1])[0][0]
    best_func  = next(f for n, f in STRATEGIES if n == best_name)
    print(f'\n  Per-ticker [{best_name}]:')
    print(f'  {"Ticker":<26} {"PnL%":>7} {"Trades":>7} {"WR%":>6} {"PF":>6}')
    print(f'  {"-"*56}')
    per_t = []
    for ticker, df_full in raw_data.items():
        df = df_full.tail(cutoff).reset_index(drop=True)
        ...
        per_t.append((ticker.replace('/USDC',''), r['pnl'], r['n'], r['wr'], r['pf']))
    for t, pnl, n, wr, pf in sorted(per_t, key=lambda x: -x[1]):
        print(f'  {t:<26} {pnl:>7.1f}%  {n:>3}     {wr*100:>4.0f}%  {pf:>6.2f}')
