...
- **Auditor re-audits without a ledger**: Supabase `trades` has NO `audited` column — `get_closed_trades(audited=False)` ignores the flag and returns the latest 100 CLOSED trades every cycle. Before 2026-06-12 the same trades were re-audited each run: trades with signals updated `agent_weights.json` again and again (compounding bias), trades without signals re-logged a skip warning forever. Fixed with a local `audited_trades.json` ID ledger (bounded at 5000). If you reset weight learning, also delete this ledger or nothing will be re-processed.
- **`pnl` is gross; `pnl_net` is real (2026-06-12)**: On close, `execution_agent` fetches actual fees (all fills since entry) and net funding from the HL ledgers via `HyperliquidExchange.get_trade_costs()` and writes `fees`, `funding_received`, `pnl_net`. The `pnl` field stays price-based gross for backward compat — all historical stats (win rate, PF) are computed on gross and are therefore flattered. Use `pnl_net` for any new profitability analysis. Records closed before 2026-06-12 have `fees≈0` and no `pnl_net`.
...
- **MONITOR deadlock**: When a ticker's score sits persistently in the MONITOR zone, the LLM keeps returning MONITOR and the ticker never executes — it can loop for 1000+ cycles. Detection: `consecutive_monitor_count` field in `ticker_state.json`. Fix in `main.py`: after 50 consecutive MONITOR cycles above the promote floor, `next_step` is automatically promoted to BUILD_CASE with a WARNING log. SwarmMonitor Check 15 alerts when this threshold is reached.
- **Decision bands are threshold-derived — keep the chain coherent (2026-06-12)**: The funnel has FOUR stacked gates that must stay aligned: (1) gate 1 `abs(score) >= threshold` (SHORT ×0.60), (2) LLM bands in `project_lead.py` — BUILD_CASE ≥ `thr+0.05`, MONITOR `thr..thr+0.05`, derived from `_effective_threshold` (NOT hardcoded — the old hardcoded 0.38/0.28 crypto bands vs `score_threshold=0.20` created a dead zone where every candidate cost an LLM call and was then rejected by instruction; zero trades Jun 7–12), (3) conviction gate `SETUP_MIN_CONVICTION` (SHORT ×0.60), (4) MONITOR auto-promote floor in `main.py` (= same band, SHORT-discounted). If you add or change any gate, apply the same SHORT discount and derive from `score_threshold` — a single hardcoded constant anywhere in the chain silently re-creates the dead zone.
...
- **SwarmMonitor: one crashing check aborts the whole round**: In `_run_checks()` an unhandled exception in any of the 17 checks propagated out, skipping all later checks AND the `_prev_check_time = now` update at the end — which then corrupted CYCLE_FROZEN detection (inflated `elapsed_min` → false alerts). Each check is now wrapped in `self._safe_check(fn, *args)`. The original crash: `agent.get("last_error", "No details")` returns **None** when `last_error` is null (a `.get` default applies only to ABSENT keys, not null values), then `None[:200]` raised `TypeError`. Use `agent.get("last_error") or "No details"`. Fixed 2026-06-07. See [[monitor-resilience]], [[python-null-falsy-gotchas]].
- **CYCLE_FROZEN false alarms on heavy cycles**: Heartbeat cycles legitimately run 6-19 min — convergence cycles (cycle_count multiple of 60) stack TreasuryAgent full run + SwarmLearner + a full scout/sentiment scan. The old check fired at ~10 min off elapsed-since-last-monitor-check. It now uses a cumulative per-agent `_cycle_last_advance` timer with threshold `CYCLE_FROZEN_THRESHOLD_MIN`=25 (env `MONITOR_CYCLE_FROZEN_MINUTES`), Heartbeat-only (Scout/auditors cycle irregularly by design; the stale-pulse check covers a truly dead agent). Fixed 2026-06-07.
- **False "Pipeline signal failure / TA crash likely" alerts**: `_check_signal_health` used `score = entry.get("score") or entry.get("weighted_score")`; a valid score of **0.0** (e.g. closed-market XYZ tickers outside 14:30-21:00 UTC) is falsy → read as None → 10-in-a-row fired a false alert. `decision_history` entries have no `weighted_score` field (the field is `score`/`decision`). Now tests `score is None` explicitly. Fixed 2026-06-07.
- **TA `fetch_data` must reuse a shared rate-limited ccxt instance**: Each call previously built a fresh `ccxt.hyperliquid()` (reloads the whole HL market list ~11.6s) with no rate limiting → 429s → empty fetches → 0.00 scores AND slow cycles (the dominant cycle-time cost). Now a module-level `_get_shared_exchange()` (markets loaded once, `enableRateLimit`) reused under `_FETCH_LOCK`, + an OHLCV cache (TTL 1d=30m/1h=5m) + 429 retry/backoff. ~33× faster (16 fetches 187s→17s), benefits crypto too. See [[ta-shared-exchange]]. Fixed 2026-06-07.
- **XYZ TA scored 0.00 for ~6 days — `calculate_indicators` returns None below 50 candles**: XYZ `_filter_market_hours` keeps only ~21% of 1h candles, so the old `fetch_limits={'1d':60,'1h':200}` left ~35 usable 1h candles → under the 50-floor → None → score 0.00 every cycle (young tokens like XYZ-MRVL also fail the 1d floor with only 35 daily candles). Fixed 2026-06-07 by bumping XYZ `fetch_limits` to `{'1d':365,'1h':500}`. Diagnose via the score distribution + "[Stock TA] … only 0 rows after filter" warnings. XYZ TA ignores the BTC regime by design — it is NOT the bull-short gate. See [[xyz-ta-data-starvation]].
- **XYZ commodities must NOT use equity market hours**: `StockTechnicalAnalyst._filter_market_hours` originally applied US-stock hours (Mon-Fri 14:30-21:00 UTC) to ALL XYZ, but commodities (XYZ-GOLD/CL/SILVER/COPPER/NATGAS/PLATINUM/PALLADIUM) trade ~23h/day (Sun 23:00 → Fri 22:00 UTC, daily 22:00-23:00 break) — equity hours threw away ~17h/day of candles (GOLD: 105→344 usable). Now asset-class-aware: `_filter_market_hours(df, asset_class)` + a `_market_is_open(asset_class, now)` entry gate; `analyze()` sets `self._asset_class`. Equities enter only 14:30-21:00 Mon-Fri; commodities ~24/5. **The gate exists in TWO places** — `ProjectLead` defers XYZ pre-TA (`project_lead.py`, returns NO_GO before calling the TA) AND the TA's `_market_is_open`. Both must be asset-class-aware or the ProjectLead gate keeps blocking commodities with equity hours. Fixed 2026-06-07.
...
- **Morpho BBQUSDC vault (0x7e97…) deposit cap**: Large deposits ($1,670) revert on-chain with unknown reason while small deposits ($432) succeed. Possibly a per-depositor cap or vault supply cap. Route large amounts to Aave v3 instead. Morpho is still listed as automated but may require investigation before large allocations. 2026-06-12: two $214–220 deposits also FAILED (RPC 403 on the dry-run eth_call, then revert) — watch the retry behaviour.
- **Liquidity guard — epoch-based vaults are never an automated deposit destination (2026-06-12)**: Protocols with `immediate_withdraw=false` in `config/treasury_protocols.json` (Gains gUSDC) are excluded as destination in ALL automated deployment paths: `_pick_best_protocol()`, `AllocationOptimizer.optimize()`, `_check_yield_switch()` and `_check_yield_diversification()`. Rationale: capital deposited there cannot be auto-withdrawn (epoch request + wait), which stalls diversification and HL rebalancing on manual action — this is exactly how 100% of yield got stuck in Gains for 11+ days. The old code redeposited $68 into Gains minutes before the guard landed (TRP_20260612_1424_2). Withdrawing FROM such vaults (manual) and reading their balance remain supported. If you add a new protocol executor, set `immediate_withdraw` honestly in the config — defaulting to liquid (absent key = true) is assumed everywhere.
